...
async fn require_fin_budget_or_legacy_manage(state: &State<'_, AppState>, user_id: i32) -> AppResult<()> {
    if check_permission(
        &state.db,
        user_id,
        crate::rbac::permissions::FIN_BUDGET,
    ...
    .await?
    {
        ...
        .await?;
        crate::license::queries::enforce_permission_matrix(&state.db, user_id, crate::rbac::permissions::FIN_BUDGET)
            .await?;
        return Ok(());
    }
    if check_permission(
        &state.db,
        user_id,
        crate::rbac::permissions::FIN_MANAGE,
        &PermissionScope::Global,
    )
    .await?
    {
        ...
        .await?;
        crate::license::queries::enforce_permission_matrix(&state.db, user_id, crate::rbac::permissions::FIN_MANAGE)
            .await?;
        return Ok(());
    }
...
async fn require_fin_post(state: &State<'_, AppState>, user_id: i32) -> AppResult<()> {
    if check_permission(
        &state.db,
        user_id,
        crate::rbac::permissions::FIN_POST,
        &PermissionScope::Global,
    )
    .await?
    {
        crate::entitlements::queries::enforce_capability_for_permission(&state.db, crate::rbac::permissions::FIN_POST)
            .await?;
        crate::license::queries::enforce_permission_matrix(&state.db, user_id, crate::rbac::permissions::FIN_POST)
            .await?;
        return Ok(());
    }
    Err(AppError::PermissionDenied("Required permission: fin.post.".to_string()))
}
...
async fn require_fin_report(state: &State<'_, AppState>, user_id: i32) -> AppResult<()> {
    if check_permission(
        &state.db,
        user_id,
        crate::rbac::permissions::FIN_REPORT,
        &PermissionScope::Global,
    )
    .await?
    {
        ...
        .await?;
        crate::license::queries::enforce_permission_matrix(&state.db, user_id, crate::rbac::permissions::FIN_REPORT)
            .await?;
        return Ok(());
    }
    Err(AppError::PermissionDenied(
...
#[tauri::command]
pub async fn list_cost_centers(filter: CostCenterFilter, state: State<'_, AppState>) -> AppResult<Vec<CostCenter>> {
...
#[tauri::command]
pub async fn create_cost_center(input: CreateCostCenterInput, state: State<'_, AppState>) -> AppResult<CostCenter> {
    let user = require_session!(state);
...
#[tauri::command]
pub async fn list_budget_lines(filter: BudgetLineFilter, state: State<'_, AppState>) -> AppResult<Vec<BudgetLine>> {
    let user = require_session!(state);
    require_permission!(
        state,
        &user,
...
#[tauri::command]
pub async fn create_budget_line(input: CreateBudgetLineInput, state: State<'_, AppState>) -> AppResult<BudgetLine> {
    let user = require_session!(state);
    require_fin_budget_or_legacy_manage(&state, user.user_id).await?;
...
pub async fn create_budget_actual(
    ...
    let user = require_session!(state);
    let allow_posted = check_permission(
        &state.db,
        user.user_id,
        crate::rbac::permissions::FIN_POST,
        &PermissionScope::Global,
...
#[tauri::command]
pub async fn post_budget_actual(input: PostBudgetActualInput, state: State<'_, AppState>) -> AppResult<BudgetActual> {
    let user = require_session!(state);
...
#[tauri::command]
pub async fn export_posted_actuals_for_erp(state: State<'_, AppState>) -> AppResult<Vec<ErpPostedActualExportItem>> {
    let user = require_session!(state);
    require_fin_report(&state, user.user_id).await?;
