...
pub async fn validate_di_origin_code(
...
pub async fn validate_di_request_type(
    db: &impl ConnectionTrait,
    request_type: &str,
) -> AppResult<String> {
...
pub async fn require_di_symptom_id(
    db: &impl ConnectionTrait,
    symptom_code_id: Option<i64>,
) -> AppResult<i64> {
    let id = symptom_code_id.ok_or_else(|| {
        AppError::ValidationFailed(vec!["Le symptôme est obligatoire.".into()])
    })?;
...
pub async fn resolve_di_symptom_id_by_code(
    db: &impl ConnectionTrait,
    code: &str,
    ...
    match row {
        Some(r) => Ok(Some(r.try_get::<i64>("", "id").map_err(|e| {
...
pub async fn resolve_system_di_symptom_id(
    db: &impl ConnectionTrait,
    preferred_code: &str,
) -> AppResult<i64> {
...
mod tests {
    ...
    async fn valid_origin_and_symptom_succeed() {
        let db = setup().await;
        let origin = validate_di_origin_code(&db, "operator")
            .await
            .expect("origin");
        assert_eq!(origin, "operator");
        let sid = symptom_id(&db, "vibration").await;
        require_di_symptom_id(&db, Some(sid))
            .await
            .expect("symptom");
    }
    ...
    async fn unknown_symptom_id_fails() {
        let db = setup().await;
        let err = validate_di_symptom_id(&db, 9_999_999)
            .await
            .expect_err("must fail");
        assert!(matches!(err, AppError::ValidationFailed(_)));
    }
    ...
    async fn null_symptom_fails_on_require() {
        let db = setup().await;
        let err = require_di_symptom_id(&db, None)
            .await
            .expect_err("must fail");
        assert!(matches!(err, AppError::ValidationFailed(_)));
    }
