//! Conversion tests (lifecycle redesign, 7-state model).
//!
//! V1 – Conversion atomicity (row-version conflict → transaction rollback).
//! V2 – State-machine guard (only `approved` can convert).
//! V3 – Missing classification → descriptive error.
//! V4 – Successful conversion → DI status=closed, disposition=converted_to_wo, WO stub created.
...
mod tests {
    ...
    /// Advance a DI through: create → in_review → screen → approve.
    /// Returns (di_id, row_version after approval).
    ...
    async fn v1_row_version_conflict_rolls_back_entire_transaction() {
        ...
        let result = convert_di_to_work_order(
            ...
            WoConversionInput {
                di_id,
                actor_id: user_id,
                expected_row_version: correct_version + 999,
        ...
        let wo_count: i64 = db
            ...
            .try_get("", "cnt")
            .expect("cnt");
        assert_eq!(wo_count, 0, "No WO must exist after rolled-back conversion");
        ...
        let log_cnt: i64 = db
            ...
            .try_get("", "cnt")
            .expect("cnt");
        assert_eq!(log_cnt, 0, "No transition log for 'convert' must exist after rollback");
    }
    ...
    async fn v2_conversion_from_submitted_fails() {
        ...
        assert!(result.is_err(), "conversion must fail when DI is in 'submitted'");
    }

    #[tokio::test]
    async fn v2_conversion_from_in_review_fails() {
        let db = setup().await;
        let user_id = get_user_id(&db).await;
        ...
        assert!(result.is_err(), "conversion must fail when DI is in 'in_review'");
    }
    ...
    async fn v2_conversion_permission_requires_step_up() {
        ...
        // If permission doesn't exist in test seed, test is vacuously correct
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // V4 – Successful conversion creates WO stub, closes DI
    // ═══════════════════════════════════════════════════════════════════════

    #[tokio::test]
    async fn v4_successful_conversion_closes_di_and_creates_wo_stub() {
        let db = setup().await;
        let user_id = get_user_id(&db).await;
        let (di_id, version) = advance_to_approved(&db, user_id).await;
        ...
        // DI must be closed with disposition converted_to_wo
        assert_eq!(result.di.status, "closed");
        assert_eq!(result.di.disposition_code.as_deref(), Some("converted_to_wo"));
        assert!(result.di.closed_at.is_some(), "closed_at must be set");
        assert_eq!(result.di.closed_by_id, Some(user_id));
        ...
        // WO row must exist with correct data
        let wo_row = db
            .query_one(Statement::from_sql_and_values(
                DbBackend::Sqlite,
        ...
        assert!(
            wo_status == "draft" || wo_status == "planning",
            "WO status must be draft/planning shell; got '{wo_status}'"
        );
    ...
    async fn v4_conversion_records_event_and_transition_log() {
        ...
        // Review event: 'converted' with step_up_used = true
    ...
    async fn v5_converted_di_rejects_draft_field_updates() {
        ...
        assert_eq!(converted.di.status, "closed");
        ...
        assert!(result.is_err(), "update_di_draft_fields must fail on a closed DI");
    ...
    async fn v5_double_conversion_fails() {
        ...
        assert!(
            result.is_err(),
            "second conversion attempt must fail — DI is already closed"
        );
    }
}
