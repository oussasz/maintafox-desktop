...
fn map_hierarchy(row: &sea_orm::QueryResult) -> AppResult<FailureHierarchy> {
    Ok(FailureHierarchy {
        ...
        asset_scope_json: row
            .try_get("", "asset_scope")
            .map_err(|e| decode_err("asset_scope", e))?,
...
fn map_code(row: &sea_orm::QueryResult) -> AppResult<FailureCode> {
    Ok(FailureCode {
        ...
        entity_sync_id: row
            ...
            .map_err(|e| decode_err("entity_sync_id", e))?,
        hierarchy_id: row
            .try_get("", "hierarchy_id")
            .map_err(|e| decode_err("hierarchy_id", e))?,
        parent_id: row
        ...
        let target_set = db
            ...
            .ok_or_else(|| {
                AppError::ValidationFailed(vec![
                    "No reference set found for WORK.FAILURE_MODES. Create/publish one in Reference Data.".into(),
                ])
            })?;
        let set_id: i64 = target_set.try_get("", "set_id").map_err(|e| decode_err("set_id", e))?;
        db.execute(Statement::from_sql_and_values(
            DbBackend::Sqlite,
...
pub async fn deactivate_failure_code(
    ...
    if current.row_version != input.expected_row_version {
        return Err(AppError::ValidationFailed(
            vec!["expected_row_version mismatch.".into()],
        ));
    }
...
fn map_cost_of_failure(row: &sea_orm::QueryResult) -> AppResult<CostOfFailureRow> {
    Ok(CostOfFailureRow {
        ...
        currency_code: row
            ...
            .map_err(|e| decode_err("currency_code", e))?,
    })
}
...
pub async fn ingest_failure_event_from_closed_wo(db: &DatabaseConnection, wo_id: i64, actor_id: i64) -> AppResult<()> {
    let wo_row = db
        .query_one(Statement::from_sql_and_values(
            DbBackend::Sqlite,
    ...
    let (failure_mode_id, failure_cause_id, failure_effect_id, cause_not_determined_i) = if let Some(r) = fd_row {
        (
...
fn map_runtime_exposure_log(row: &sea_orm::QueryResult) -> AppResult<RuntimeExposureLog> {
    Ok(RuntimeExposureLog {
        ...
        equipment_id: row
            ...
            .map_err(|e| decode_err("equipment_id", e))?,
        exposure_type: row
            .try_get("", "exposure_type")
            .map_err(|e| decode_err("exposure_type", e))?,
...
fn map_kpi_snapshot(row: &sea_orm::QueryResult) -> AppResult<ReliabilityKpiSnapshot> {
    Ok(ReliabilityKpiSnapshot {
        ...
        mtbf: row
            .try_get::<Option<f64>>("", "mtbf")
            .map_err(|e| decode_err("mtbf", e))?,
        mttr: row
            .try_get::<Option<f64>>("", "mttr")
            .map_err(|e| decode_err("mttr", e))?,
        availability: row
            .try_get::<Option<f64>>("", "availability")
            .map_err(|e| decode_err("availability", e))?,
        failure_rate: row
        ...
        event_count: row
            .try_get("", "event_count")
            .map_err(|e| decode_err("event_count", e))?,
...
async fn kpi_refresh_core(
    ...
    if p1 < p0 {
        return Err(AppError::ValidationFailed(vec![
            "period_end before period_start.".into()
        ]));
    }
    ...
    for r in &fe_rows {
        ...
        failure_parts.push(FailurePart {
            ...
            downtime_duration_hours: r
                .try_get("", "downtime_duration_hours")
                .map_err(|e| decode_err("downtime_duration_hours", e))?,
            active_repair_hours: r
                .try_get("", "active_repair_hours")
                .map_err(|e| decode_err("active_repair_hours", e))?,
            failure_mode_id: r
                .try_get::<Option<i64>>("", "failure_mode_id")
...
fn map_ram_issue(row: &sea_orm::QueryResult) -> AppResult<RamDataQualityIssue> {
    Ok(RamDataQualityIssue {
        ...
        remediation_url: row
            .try_get("", "remediation_url")
            .map_err(|e| decode_err("remediation_url", e))?,
    })
}
...
fn map_user_dismissal_row(row: &sea_orm::QueryResult) -> AppResult<UserDismissal> {
    Ok(UserDismissal {
        ...
        dismissed_at: row
            ...
            .map_err(|e| decode_err("dismissed_at", e))?,
        row_version: row
            .try_get("", "row_version")
            .map_err(|e| decode_err("row_version", e))?,
    })
...
pub async fn list_ram_data_quality_issues(
    ...
    sql.push_str(" ORDER BY CASE v.severity WHEN 'blocking' THEN 0 ELSE 1 END, v.equipment_id, v.issue_code");
    let rows = db
        .query_all(Statement::from_sql_and_values(DbBackend::Sqlite, sql, vals))
        .await?;
...
pub async fn get_ram_equipment_quality_badge(
    ...
    let score_row = db
        ...
        .await?;
    let data_quality_score: Option<f64> =
        score_row.and_then(|r| r.try_get::<Option<f64>>("", "data_quality_score").ok().flatten());
