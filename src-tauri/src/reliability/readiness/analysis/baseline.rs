...
pub fn compute_baseline_comparison(
    ...
    let eligible = doc_reports.iter().filter(|r| r.eligible_event_count > 0).count() as u64;
    ...
    let naive_mtbf_sample = doc_reports.iter().filter(|r| r.eligible_event_count >= n_min).count() as u64;
    let framework_primary = doc_reports.iter().filter(|r| r.documentary_ready).count() as u64;
    let framework_strict = strict_reports.iter().filter(|r| r.strict_ready).count() as u64;
    ...
    let scenarios = vec![
        BaselineScenario {
            ...
            description: "Asset enters analysis if at least one UPM (unplanned) work order exists.".to_string(),
        },
        BaselineScenario {
            ...
            description: format!("Asset has at least N_f >= {n_min} eligible events (no completeness/DQ gate)."),
        },
        BaselineScenario {
            ...
            description: "Passes documentary readiness (DQ + completeness + no blocking issues).".to_string(),
        },
        BaselineScenario {
            ...
            description: "Passes strict RAM readiness (exposure + analysis_ready + green badge).".to_string(),
        },
    ];
