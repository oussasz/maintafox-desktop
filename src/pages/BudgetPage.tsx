...
export function BudgetPage() {
  ...
  const [erpReforecastPayload, setErpReforecastPayload] = useState<ErpApprovedReforecastExportItem[]>([]);
  ...
  const selectedLine = useMemo(() => lines.find((item) => item.id === selectedLineId) ?? null, [lines, selectedLineId]);
  ...
  async function loadVersionLines(versionId: number | null) {
    ...
    try {
      ...
      setSelectedLineId((current) => (current && loadedLines.some((item) => item.id === current) ? current : null));
    } catch (err) {
      setError(toErrorMessage(err));
    }
  }
  ...
  async function loadVersionFinanceLayers(versionId: number | null) {
    ...
    try {
      ...
      setSelectedActualId((current) =>
        current && loadedActuals.some((item) => item.id === current) ? current : loadedActuals[0]?.id ?? null,
      );
      setSelectedForecastId((current) =>
        current && loadedForecasts.some((item) => item.id === current) ? current : loadedForecasts[0]?.id ?? null,
      );
      setSelectedVarianceReviewId((current) =>
        current && loadedReviews.some((item) => item.id === current) ? current : loadedReviews[0]?.id ?? null,
      );
      setSelectedAlertEventId((current) =>
        current && loadedAlertEvents.some((item) => item.id === current) ? current : loadedAlertEvents[0]?.id ?? null,
      );
    } catch (err) {
      setError(toErrorMessage(err));
    }
  ...
  useEffect(() => {
    if (!selectedLine) {
      setLineForm((current) => ({
        ...EMPTY_LINE_FORM,
        cost_center_id:
          selectedCostCenterId !== null ? String(selectedCostCenterId) : current.cost_center_id ?? "__none__",
      }));
      return;
    }
    setLineForm({
  ...
  async function handleUpdateCostCenter() {
    ...
    setError(null);
    try {
      const updated = await updateCostCenter(selectedCostCenter.id, selectedCostCenter.row_version, {
        code: costCenterForm.code,
        name: costCenterForm.name,
        entity_id: parseOptionalNumber(costCenterForm.entity_id),
        parent_cost_center_id: parseOptionalNumber(costCenterForm.parent_cost_center_id),
  ...
  async function handleCreateVarianceReview() {
    ...
    setError(null);
    try {
      const varianceAmount = (selectedLine.planned_amount * 0.12) * (selectedLine.budget_bucket === "labor" ? 1.1 : 1);
      const variancePct = selectedLine.planned_amount === 0 ? 0 : (varianceAmount / selectedLine.planned_amount) * 100;
      await createBudgetVarianceReview({
        ...
        action_owner_id: 1,
        review_commentary: "Variance opened from dashboard drilldown with accountable owner and coded driver.",
        snapshot_context_json: JSON.stringify({
          ...
          commitments: commitments.filter((item) => item.cost_center_id === selectedLine.cost_center_id).length,
          actuals: actuals.filter((item) => item.cost_center_id === selectedLine.cost_center_id).length,
          forecasts: forecasts.filter((item) => item.cost_center_id === selectedLine.cost_center_id).length,
        }),
      });
      await loadVersionFinanceLayers(selectedVersion.id);
    } catch (err) {
  ...
  async function handleAdvanceVarianceReview(nextStatus: string) {
    ...
    try {
      await transitionBudgetVarianceReview({
        ...
        reopen_reason: nextStatus === "open" ? "Material posting change triggered automatic reassessment." : null,
      });
      await loadVersionFinanceLayers(selectedVersionId);
    } catch (err) {
      setError(toErrorMessage(err));
  ...
  async function handleRefreshErpPayloads() {
    ...
    setError(null);
    try {
      const [posted, reforecasts] = await Promise.all([
        exportPostedActualsForErp(),
        exportApprovedReforecastsForErp(),
      ]);
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Budget baseline authoring</h1>
          <p className="text-sm text-muted-foreground">
            Govern cost center hierarchy, versioned budget baselines, and traceable period-aware budget lines.
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => void loadBaseline()} disabled={loading || saving}>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.1fr_1.2fr_1.5fr]">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Select a node, then update or create governed children.</CardDescription>
              </CardHeader>
                ...
                <div className="grid gap-3">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setCostCenterForm((current) => ({ ...current, code: e.target.value }))}
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setCostCenterForm((current) => ({ ...current, name: e.target.value }))}
                      disabled={!canBudget || saving}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => setCostCenterForm((current) => ({ ...current, entity_id: value }))}
                      disabled={!canBudget || saving}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      ...
                      <SelectContent>
                        ...
                        {orgNodes.map((item) => (
                          <SelectItem key={item.node.id} value={String(item.node.id)}>
                            {item.node.code ? `${item.node.name} (${item.node.code})` : item.node.name}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setCostCenterForm((current) => ({ ...current, parent_cost_center_id: value }))
                      }
                      disabled={!canBudget || saving}
                    >
                      <SelectTrigger>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCostCenterForm((current) => ({ ...current, erp_external_id: e.target.value }))
                      }
                      disabled={!canBudget || saving}
                    />
                  </div>
                  <div className="flex gap-2">
                    <Button onClick={() => void handleCreateCostCenter()} disabled={!canBudget || saving}>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Lifecycle-aware budget baseline governance with successor creation.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="max-h-72 overflow-auto rounded-md border">
                  <Table>
                ...
                <div className="grid gap-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setVersionForm((current) => ({ ...current, fiscal_year: e.target.value }))}
                        disabled={!canBudget || saving}
                      />
                    </div>
                    <div className="grid gap-2">
                    ...
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setVersionForm((current) => ({ ...current, currency_code: e.target.value }))
                        }
                        disabled={!canBudget || saving}
                      />
                    </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setVersionForm((current) => ({ ...current, title: e.target.value }))}
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setVersionForm((current) => ({ ...current, planning_basis: e.target.value }))
                      }
                      disabled={selectedVersion ? !versionEditable || saving : !canBudget || saving}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setVersionForm((current) => ({ ...current, baseline_reference: e.target.value }))
                      }
                      disabled={selectedVersion ? !versionEditable || saving : !canBudget || saving}
                    />
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <Button onClick={() => void handleCreateVersion()} disabled={!canBudget || saving}>
                      Create version
                    </Button>
                    <Button
                      variant="outline"
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      disabled={!selectedVersion || selectedVersion.status !== "submitted" || !canBudget || saving}
                    >
                      Approve
                    </Button>
                    <Button
                      onClick={() => void handleTransitionVersion("frozen")}
                      disabled={!selectedVersion || selectedVersion.status !== "approved" || !canBudget || saving}
                    >
                      Freeze baseline
                    </Button>
                  </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Period-aware budget lines remain editable only while the selected version is still draft.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="max-h-72 overflow-auto rounded-md border">
                    ...
                    <TableBody>
                      {lines.map((item) => (
                        ...
                        >
                          ...
                          <TableCell className="text-right">{item.planned_amount.toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                ...
                <div className="grid gap-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setLineForm((current) => ({ ...current, cost_center_id: value }))}
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setLineForm((current) => ({ ...current, period_month: value }))}
                        disabled={!versionEditable || saving}
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setLineForm((current) => ({ ...current, budget_bucket: value }))}
                        disabled={!versionEditable || saving}
                      >
                        <SelectTrigger>
                          <SelectValue />
                    ...
                    <div className="grid gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => setLineForm((current) => ({ ...current, labor_lane: value }))}
                        disabled={!versionEditable || saving}
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setLineForm((current) => ({ ...current, source_basis: e.target.value }))}
                      disabled={!versionEditable || saving}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setLineForm((current) => ({ ...current, justification_note: e.target.value }))
                      }
                      disabled={!versionEditable || saving}
                    />
                  </div>
                  <div className="flex gap-2">
                    <Button onClick={() => void handleCreateLine()} disabled={!versionEditable || saving}>
                      Create line
                    </Button>
                    <Button
                      variant="outline"
          ...
          <div className="grid gap-6 xl:grid-cols-[1.1fr_1.1fr_1.3fr]">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    disabled={!canPost || !selectedActual || selectedActual.posting_status !== "provisional" || saving}
                  >
                  ...
                  <Button
                    ...
                    disabled={!canPost || !selectedActual || selectedActual.posting_status !== "posted" || saving}
                  >
                    ...
                    <TableBody>
                      {actuals.map((item) => (
                        ...
                        >
                          ...
                          <TableCell className="text-right">{item.amount_base.toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>PO and contract obligations remain separate from actual costs.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <Button
                    ...
                    <TableBody>
                      {commitments.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>{item.source_type}</TableCell>
                          <TableCell className="text-right">{item.base_amount.toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Idempotent reruns with PM/backlog/shutdown/planning drivers and confidence metadata.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  <Button onClick={() => void handleGenerateForecasts()} disabled={!canBudget || !selectedVersion || saving}>
                    ...
                    <TableBody>
                      {forecasts.map((item) => (
                        ...
                        >
                          ...
                          <TableCell className="text-right">{item.forecast_amount.toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                ...
                <div className="rounded-md border p-3">
                  <div className="text-xs font-medium uppercase text-muted-foreground">Selected forecast explainability</div>
                  <pre className="mt-2 overflow-auto text-xs">
                    {selectedForecast ? decodeOptionalJson(selectedForecast.explainability_json) : "—"}
                  </pre>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Expose the baseline assumptions that later forecasts and actuals will reconcile against.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {selectedVersion ? (
                  <>
                    <div className="flex flex-wrap gap-2">
                      <Badge variant={budgetStatusVariant(selectedVersion.status)}>{selectedVersion.status}</Badge>
                    ...
                    <div className="grid gap-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setVersionForm((current) => ({ ...current, source_basis_mix_json: e.target.value }))
                        }
                        disabled={!versionEditable || saving}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setVersionForm((current) => ({ ...current, labor_assumptions_json: e.target.value }))
                        }
                        disabled={!versionEditable || saving}
                      />
                    </div>
                    <div className="grid gap-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setVersionForm((current) => ({ ...current, erp_external_ref: e.target.value }))
                        }
                        disabled={!versionEditable || saving}
                      />
                    </div>
                    <Button onClick={() => void handleUpdateVersion()} disabled={!versionEditable || saving}>
                      Save explainability fields
                    </Button>
                  </>
                ) : (
                  <p className="text-sm text-muted-foreground">Select a budget version to inspect its governed baseline evidence.</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Read-only snapshot of what the chosen baseline currently communicates.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {selectedVersion ? (
                  <>
                    <div className="rounded-md border p-3">
                      <div className="text-xs font-medium uppercase text-muted-foreground">Planning basis</div>
                      <div className="mt-1 text-sm">{selectedVersion.planning_basis ?? "—"}</div>
                    </div>
                    <div className="rounded-md border p-3">
                      <div className="text-xs font-medium uppercase text-muted-foreground">Source basis mix</div>
                      <pre className="mt-2 overflow-auto text-xs">{decodeOptionalJson(selectedVersion.source_basis_mix_json)}</pre>
                    </div>
                    <div className="rounded-md border p-3">
                      <div className="text-xs font-medium uppercase text-muted-foreground">Labor assumptions</div>
                      <pre className="mt-2 overflow-auto text-xs">{decodeOptionalJson(selectedVersion.labor_assumptions_json)}</pre>
                    </div>
                    <div className="rounded-md border p-3">
                      <div className="text-xs font-medium uppercase text-muted-foreground">Baseline reference</div>
                      <div className="mt-1 text-sm">{selectedVersion.baseline_reference ?? "—"}</div>
                    </div>
                    <div className="rounded-md border p-3">
                      <div className="text-xs font-medium uppercase text-muted-foreground">ERP linkage</div>
                      <div className="mt-1 text-sm">{selectedVersion.erp_external_ref ?? "local only"}</div>
                    </div>
                  </>
                ) : (
          ...
          <div className="grid gap-6 xl:grid-cols-3">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Governed lifecycle with coded drivers, owner accountability, and reproducible snapshot context.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  <Button onClick={() => void handleCreateVarianceReview()} disabled={!canBudget || !selectedLine || saving}>
                    ...
                    <TableBody>
                      {varianceReviews.map((item) => (
                        ...
                        >
                          ...
                          <TableCell className="text-right">{item.variance_amount.toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                ...
                <div className="rounded-md border p-3">
                  <div className="text-xs font-medium uppercase text-muted-foreground">Snapshot context</div>
                  <pre className="mt-2 overflow-auto text-xs">
                    {selectedVarianceReview ? decodeOptionalJson(selectedVarianceReview.snapshot_context_json) : "—"}
                  </pre>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Planned, committed, actual, and forecast layers by spend mix, team, assignee, and labor lane.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="max-h-64 overflow-auto rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {dashboardRows.map((row, idx) => (
                        <TableRow key={`${row.cost_center_id}-${row.period_month ?? "annual"}-${row.budget_bucket}-${idx}`}>
                          <TableCell>{row.spend_mix}</TableCell>
                          <TableCell>{row.team_id ?? "—"}</TableCell>
                          <TableCell>{row.labor_lane ?? "—"}</TableCell>
                          <TableCell className="text-right">{row.planned_amount.toFixed(2)}</TableCell>
                          <TableCell className="text-right">{row.actual_amount.toFixed(2)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                    ...
                    <TableBody>
                      {drilldownRows.slice(0, 24).map((row) => (
                        <TableRow key={`${row.layer_type}-${row.record_id}`}>
                          ...
                          <TableCell>{row.work_order_id ?? row.pm_occurrence_ref ?? row.inspection_ref ?? "—"}</TableCell>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Optional ERP master import plus export-ready posted actual and approved reforecast payloads.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <PermissionGate permission="fin.report">
                  <div className="flex flex-wrap gap-2">
                    <Button variant="outline" onClick={() => void handleImportErpMaster()} disabled={saving}>
                      Import ERP master
                    </Button>
                ...
                <div className="rounded-md border p-3">
                  <div className="text-xs font-medium uppercase text-muted-foreground">Alert controls</div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <Button
                      variant="outline"
                      onClick={() => void handleEnsureDefaultAlertRules()}
                      disabled={!canBudget || !selectedVersion || selectedVersion.status !== "frozen" || saving}
                    >
                  ...
                  <div className="mt-2 text-xs text-muted-foreground">
                    Rules: {alertConfigs.length} | Events: {alertEvents.length}
                  </div>
                </div>
                ...
                <div className="rounded-md border p-3">
                  <div className="text-xs font-medium uppercase text-muted-foreground">Report packs</div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <Button variant="outline" onClick={() => void handleBuildReportPack()} disabled={!selectedVersion || saving}>
                      Build explainable pack
                    </Button>
                    <Button onClick={() => void handleExportReportPack("pdf")} disabled={!canReport || !selectedVersion || saving}>
                      Export PDF pack
                    </Button>
                    <Button variant="outline" onClick={() => void handleExportReportPack("excel")} disabled={!canReport || !selectedVersion || saving}>
                      Export Excel pack
                    </Button>
                  </div>
                  {reportPack ? (
                    <div className="mt-2 text-xs text-muted-foreground">
                      Baseline {reportPack.totals.baseline_amount.toFixed(2)} | Actual {reportPack.totals.posted_actual_amount.toFixed(2)} | Variance {reportPack.totals.variance_amount.toFixed(2)} ({reportPack.totals.variance_pct.toFixed(2)}%)
                    </div>
                  ) : null}
                  {reportExport ? (
                ...
                {!canReport ? (
                  <p className="text-sm text-muted-foreground">
                    ERP import/export actions require `fin.report` permission. Dashboard and reviews remain visible under `fin.view`.
                  </p>
                ) : null}
      ...
      {loading ? <p className="text-sm text-muted-foreground">Loading finance baseline workspace...</p> : null}
    </div>
  );
}
