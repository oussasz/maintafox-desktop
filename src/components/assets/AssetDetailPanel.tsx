            ...
            <div className="min-w-0 space-y-1.5">
              ...
              {(asset.manufacturer?.trim() || asset.model?.trim()) && (
                <p className="text-xs text-text-muted">
                  {[asset.manufacturer?.trim(), asset.model?.trim()].filter(Boolean).join(" · ")}
                </p>
              )}
                ...
                <PermissionGate permission={P.EQ_MANAGE}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="w-48">
                      <DropdownMenuItem className="gap-2" onClick={() => openEditForm(asset)}>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <Row
                ...
                value={asset.family_name ?? asset.family_code}
              />
                      ...
                      <li key={row.relation_id} className="flex items-start gap-2 text-xs">
                        ...
                        <div>
                          <span className="font-mono">
                            {child?.asset_code ?? row.child_asset_id}
                          </span>
                          {child && (
                            <span className="block text-text-muted">{child.asset_name}</span>
                          )}
                        </div>
                      </li>
                    ))}
