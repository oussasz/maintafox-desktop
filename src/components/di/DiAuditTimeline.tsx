...
export function DiAuditTimeline({ diId }: DiAuditTimelineProps) {
  ...
  const applyResultBadge = (result: string) => {
    switch (result) {
      case "applied":
        return (
          <Badge className="bg-green-100 text-green-800 hover:bg-green-100">
            {t("auditTimeline.resultApplied")}
          </Badge>
        );
      case "blocked":
        return (
          <Badge className="bg-red-100 text-red-800 hover:bg-red-100">
            {t("auditTimeline.resultBlocked")}
          </Badge>
        );
      case "partial":
        return (
          <Badge className="bg-yellow-100 text-yellow-800 hover:bg-yellow-100">
            {t("auditTimeline.resultPartial")}
          </Badge>
        );
      default:
  ...
  const formatActedAt = (iso: string) => {
    try {
  ...
  if (events.length === 0) {
    return (
      <p className="py-6 text-center text-sm text-muted-foreground">{t("auditTimeline.empty")}</p>
    );
  }
